...
const completedKeys = (s: ActivationState) =>
  s.steps.filter((x) => x.completed).map((x) => x.key);
...
test.describe("activation — signup to Aha", () => {
  ...
  test("a newcomer reaches the Aha moment, and the checklist is server state", async ({
    ...
    let activation: ActivationState = await (
      await api.get("/activation/state")
    ).json();
    expect(
      completedKeys(activation),
      "a new tenant has completed nothing",
    ).toEqual([]);
    ...
    expect(suggestions.risks, "exactly three first-risk drafts").toHaveLength(
      3,
    );
    expect(
      suggestions.frameworks[0],
      "the chosen goal leads the framework list",
    ).toBe("cobac");
    ...
    const importRes = await api.post(
      `/compliance/frameworks/${framework.id}/import-catalog`,
      {
        catalog_key: "cobac",
      },
    );
    ...
    expect(
      imported.imported,
      "the import creates real controls",
    ).toBeGreaterThan(0);
    ...
    await expect(
      app,
      "the app is reachable once onboarding is complete",
    ).not.toHaveURL(/\/onboarding/, { timeout: 15_000 });
    ...
    expect(
      riskRes.status(),
      `first risk should be created: ${await riskRes.text()}`,
    ).toBeLessThan(300);

    activation = await (await api.get("/activation/state")).json();
    expect(completedKeys(activation)).toContain("first_risk");
    const firstRisk = activation.steps.find((s) => s.key === "first_risk")!;
    expect(firstRisk.primary, "the first risk is the product promise").toBe(
      true,
    );
    expect(
      firstRisk.celebrate,
      "the server asks for a celebration exactly once",
    ).toBe(true);
    ...
    await expect
      .poll(
        ...
        {
          message:
            "the Aha moment should be recorded once the score meets its definition",
          timeout: 20_000,
        },
      )
      .not.toBeNull();
    ...
    const afterReload: ActivationState = await (
      await api.get("/activation/state")
    ).json();
    expect(
    ...
    const fromOtherDevice: ActivationState = await (
      await api.get("/activation/state")
    ).json();
    expect(
  ...
  test("a completed step never asks to celebrate twice", async ({}) => {
    ...
    let state: ActivationState = await (
      await api.get("/activation/state")
    ).json();
    ...
    expect(
      (
        await api.post("/activation/celebrated", { step_key: "profile" })
      ).status(),
    ).toBe(204);
    expect(
      (
    ...
    expect(
      (
        await api.post("/activation/celebrated", { step_key: "not-a-step" })
      ).status(),
  ...
  test("the wizard is resumable: answers survive leaving and coming back", async ({
    browser,
  }) => {
    const ctx = await pwRequest.newContext();
    ...
    const suggestions = await (await api.get("/onboarding/suggestions")).json();
    expect(
      suggestions.risks.some(
        (r: { key: string }) => r.key === "patient_data_leak",
      ),
    ).toBe(true);
...
test.describe("posture reveal — the Aha moment (#438)", () => {
  ...
  test("an empty posture is refused, records nothing, and measures nothing", async () => {
    ...
    // And nothing was written. `posture.revealed` is a non-checklist key, so it
    // never appears in the checklist — the honest place to look is the Aha
    // anchor, which the reveal is what now sets.
    const activation: ActivationState = await (
      await api.get("/activation/state")
    ).json();
    expect(
      activation.aha_reached_at,
      "a refused reveal must not record an Aha moment",
    ).toBeNull();

    // Asking again must still refuse: a repeated refusal must not accumulate
    // into a success.
  ...
  test("a revealed posture is computed from the tenant own rows, and is idempotent", async ({}, info) => {
    ...
    const riskRes = await api.post("/risks", {
      title: "Compromission d'un compte à privilèges",
      description: "Un compte administrateur est compromis par hameçonnage.",
      probability: 0.4,
      impact: 8,
      source: "starter",
    });
    expect(
      riskRes.status(),
      `the starter risk should be created: ${await riskRes.text()}`,
    ).toBeLessThan(300);
    ...
    expect(
      revealRes.status(),
      `the posture should reveal: ${await revealRes.text()}`,
    ).toBe(200);
    ...
    expect(
      posture.coverage_percent,
      "a non-null coverage percentage",
    ).not.toBeNull();
    expect(
      posture.top_risks.length,
      "at least one residual score",
    ).toBeGreaterThan(0);
    expect(posture.top_risks[0].residual.value).toBeGreaterThanOrEqual(0);

    // Criterion 7: the numbers are THIS tenant's. A fresh tenant has exactly the
    // one risk we just created, so any other count would mean the payload is not
    // reading our rows.
    expect(posture.risks.total, "the risk count is the tenant's own").toBe(1);
    expect(posture.top_risks[0].title).toContain("privilèges");
    expect(
      posture.controls.total,
      "the imported catalogue is the tenant own",
    ).toBeGreaterThan(0);
    expect(
      posture.residual_formula_version,
      "the formula version is stamped",
    ).toBe("residual-v1");

    // No control is mapped to this risk yet, so the residual must EQUAL the
    ...
    expect(posture.first_reveal, "the first render is the first reveal").toBe(
      true,
    );
    ...
    expect(second.first_reveal, "the second render is not a first reveal").toBe(
      false,
    );
    expect(second.revealed_at, "revealed_at must not move").toBe(
      posture.revealed_at,
    );

    const elapsed = Date.now() - t0;
    info.annotations.push({
      type: "time-to-reveal-ms",
      description: String(elapsed),
    });
    expect(
      elapsed,
  ...
  test("a tenant that already holds data is recognised and skips the tunnel", async () => {
    ...
    expect(before.recognised, "a brand-new tenant is not recognised").toBe(
      false,
    );
    expect(before.skip_tunnel, "a brand-new tenant walks the tunnel").toBe(
      false,
    );
    expect(
    ...
    expect(after.recognised, "a tenant holding a risk is recognised").toBe(
      true,
    );
  ...
  test("a step whose data already exists is absent from the tunnel", async () => {
    ...
    // Answering the organization step writes industry + size onto the real
    // Organization row, which is exactly what the auto-skip probe reads.
    await api.put("/onboarding/steps/organization", {
      answers: {
        name: "Banque Atlantique CM",
        industry: "banking",
    ...
    expect(wizard.skipped_steps, "and must be reported as skipped").toContain(
      "organization",
    );
    expect(
      wizard.steps.length,
      "the stepper count shown to this user shrinks with it",
    ).toBe(4);
    expect(
      wizard.current_step,
  ...
  test("the tunnel offers no way out: no dismiss control, and Escape does not exit", async ({
    ...
    await page.goto("/", { waitUntil: "domcontentloaded" });
    await expect(page, "the tunnel is entered automatically").toHaveURL(
      /\/onboarding\//,
      {
        timeout: 15_000,
      },
    ...
    // and no close/later affordance replaced it anywhere in the tunnel.
    await expect(page.getByTestId("wizard-skip")).toHaveCount(0);
    for (const label of [
      /passer cette étape/i,
      /skip this step/i,
      /plus tard/i,
      /^later$/i,
    ]) {
    ...
    // Escape must not be a way out. Nothing in the tunnel registers a handler
    // for it; this asserts that stays true.
    ...
    await expect(
      page,
      "the app is unreachable until the tunnel completes",
    ).toHaveURL(/\/onboarding\//, { timeout: 15_000 });
