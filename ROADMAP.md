...
| **17.6 Onboarding — tunnel guidé + Révélation de posture** | 🟡 | **Tunnel bloquant** (#438) : aucun contrôle de fermeture, d'abandon ni d'échappement ; `OnboardingGuard` interdit `/app` jusqu'à `POST /onboarding/complete`. Un seul `GET /onboarding/state` résout tout le tunnel, décisions d'auto-skip comprises — une étape dont la donnée existe déjà n'est jamais rendue et disparaît du compteur (`internal/application/activation/onboarding.go`, `internal/domain/posture.go`). **Écran terminal `/posture`** : compté à partir des seules lignes du tenant, enregistre `posture.revealed` une fois (`internal/application/activation/posture.go`, `internal/infrastructure/repository/gorm_posture_repository.go`). **Écran `/onboarding/recognition`** pour les tenants déjà configurés. Score résiduel additif depuis `risk_control_mappings` (`pkg/scoring/residual.go`, ADR 0003). Catalogue de risques de démarrage FR+EN (`pkg/onboarding/starter.go`). Checklist d'activation à 7 étapes inchangée, désormais **masquable** par l'utilisateur. | Promesse d'activation : **Aha en < 8 min**, mesuré désormais sur `posture.revealed` et non plus sur le score du tableau de bord exécutif (D-010 ; série `openrisk_time_to_aha_seconds{aha_definition="v2"}`, la v1 est gelée). 🟡 et non ✅ : **la suite E2E n'a pas été exécutée** dans l'environnement de développement, et **ADR 0003 est `proposed`** — rien n'est encore persisté dans `Risk.ResidualRisk` tant que ses constantes ne sont pas validées. Étapes « intégrations » et « première évaluation » : toujours hors périmètre (#234, #438). |
