...
- [x] **W1-05a — Vues de tableau sauvegardées côté serveur** (#580, enfant A de l'épique #235).
  Une vue nommée d'un registre (recherche, facettes, tri, disposition des colonnes) vivait dans le
  `localStorage` d'un seul navigateur : impossible à donner à un collègue, perdue en vidant les données
  du site. Elle vit désormais en base, rattachée au tenant, et peut être **partagée avec toute
  l'organisation**.
  **Backend livré** : `domain.SavedView` (+ `SavedViewState` / `SavedViewColumns` en `jsonb`,
  visibilité `personal` | `shared`, plafonds sur la charge utile) enregistré dans `AutoMigrate`
  (`cmd/server/main.go`) — **purement additif, aucune colonne supprimée ni renommée, aucun fichier
  `.sql`** (la couche golang-migrate ne porte que ce qu'AutoMigrate ne sait pas exprimer, cf.
  `internal/migrations/migrator.go:64-66`). `internal/application/savedview/` (un cas d'usage par
  fichier), `internal/infrastructure/repository/gorm_saved_view_repository.go`,
  `internal/handler/saved_view_handler.go`, routes `GET|POST /saved-views` et
  `PATCH|DELETE /saved-views/:id`.
  **Isolation** : `tenant_id` dans le WHERE de **chaque** requête, lectures comme écritures. Une vue
  d'un autre tenant — et la vue **personnelle** d'un autre utilisateur du même tenant — répondent
  **404**, réponse identique à celle d'un id fabriqué : un 403 confirmerait l'existence de la ligne, et
  les valeurs de filtre d'une vue nomment les actifs, les tags et les personnes de cette institution.
  Le 403 n'apparaît que sur une vue **partagée** que l'appelant voit déjà, et seulement s'il n'en est
  ni le propriétaire ni administrateur du tenant. Décisions consignées dans
  `internal/security/isolation/registry.go`.
  **Frontend livré** : `src/services/savedViewService.ts` (types importés d'`openapi.generated.ts`,
  jamais saisis à la main ; zéro `any`) ; `useSavedViews` réécrit contre l'API dans
  `shared/datatable/useTableState.ts` ; picker de vues dans `shared/datatable/FilterPanel.tsx` avec
  les trois états (skeleton / erreur + réessai / vide se lisant « aucune pour l'instant » et non comme
  un échec), bascule personnel↔partagé par ligne, attribution du propriétaire, mutations optimistes
  avec rollback. Zod valide le formulaire ; FR + EN dans `src/locales` (`savedViews.*`).
  **Migration du `localStorage`** : elle ne détruit rien. La copie locale n'est effacée qu'une fois
  **chaque** vue confirmée stockée ; un seul POST en échec laisse l'ensemble intact et le rejoue au
  chargement suivant. Un 409 compte comme un succès, puisque c'est précisément ce que produit ce
  rejeu. Les vues migrées le sont en `personal` — une vue qui vivait dans un seul navigateur était par
  construction privée.
  **Vérifié** : `go build ./... && go vet ./...` verts ; 7 tests dépôt + 9 tests cas d'usage
  (`TestSavedViews_Success` / `_NotFound` / `_Unauthorized` + le test explicite cross-tenant
  404-et-non-403) ; 15 tests frontend dont trois passes axe-core sur le picker (rempli, vide, en
  erreur), zéro violation serious/critical ; `tsc` propre, eslint propre, suite front 494/494.
  **Reste à faire**
  - Les 4 routes `/saved-views` ont été ajoutées à `routesWithoutPermissionGuard`
    (`internal/handler/authz_route_coverage_test.go`) : aucune permission existante ne convient à une
    ressource partagée par les sept registres, et une vue ne confère aucun accès. Cette liste
    **exige l'accord d'un relecteur** — à confirmer en revue de la PR.
  - Ligne dans `docs/MARKETING_CLAIM_MATRIX.md` : **non ajoutée**, comme pour W1-03. Le statut
    `VERIFIED` est réservé à `product-verifier` via `/verify-claims`.
  - Jamais exercé contre un backend vivant : les tests front bouchonnent à la frontière HTTP, et
    l'`AutoMigrate` de `saved_views` n'a pas été joué contre un PostgreSQL réel (les tests dépôt
    tournent sur SQLite en mémoire).
  - Pas de test e2e Playwright sur le picker ; les règles axe qui exigent une géométrie (contraste,
    taille de cible) ne tournent pas en jsdom.
